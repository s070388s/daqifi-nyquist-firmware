...
#include "../src/HAL/Announce/DAQiFi_tcpip_announce.h"  // DAQiFi Modification
...
#endif
static bool                 sendDAQiFiAnnounce = false; // DAQiFi Modification
...
static void TCPIP_ANNOUNCE_Send(void)
{
    ...
    for(netIx = 0; netIx < announceIfs; netIx++)
    {
        ...
        if((announceRequestMask & (1 << netIx)) != 0 || sendDAQiFiAnnounce) // DAQiFi Modification
        {
            pNetIf = (TCPIP_NET_IF*)TCPIP_STACK_IndexToNet(netIx);
            DAQiFi_TCPIP_ANNOUNCE_Update (pNetIf);   // DAQiFi Modification
            ...
            {   // reply only if this interface is up and running
                ...
                if(sendDAQiFiAnnounce)  // Begin DAQiFi Modification
                {   
                    
                    dataLen = DAQiFi_TCPIP_ANNOUNCE_Send(buffer, sizeof(buffer));
                    minimumDataLen = dataLen;
                    
                }                       // End DAQiFi Modification
                ...
                {
                    TCPIP_UDP_ArrayPut(announceSocket, (const uint8_t *)buffer, dataLen);
                    sendDAQiFiAnnounce = false;
                }
...
static void TCPIP_ANNOUNCE_Timeout(void)
{
	...
	{   // consume all queued packets
        ...
        {   // We received a discovery request, reply
            sendDAQiFiAnnounce = false; // DAQiFi Modification
        ...
        if(memcmp(discQuery, (const void *)"DAQiFi?", sizeof(discQuery))==0)    // Begin DAQiFi modification
        {
            // We received a DAQiFi message
            sendDAQiFiAnnounce = true;
            TCPIP_ANNOUNCE_Send();
...
static void ANNOUNCE_Notify(TCPIP_NET_HANDLE hNet, uint8_t evType, const void * param)
{
// DAQiFi Modification - Don't perform Microchip announce functions as it confuses the DAQifi app
//    if(announceMemH)
//    {
