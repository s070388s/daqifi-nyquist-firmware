...
void SYS_Initialize ( void* data )
{
    portDISABLE_INTERRUPTS()    // Re enable them after starting task scheduler
    ...
    // Timer for streaming/ADC, DIO, etc. sample triggering
    sysObj.drvTmr1 = DRV_TMR_Initialize(DRV_TMR_INDEX_1, (SYS_MODULE_INIT *)&drvTmr1InitData);
    // Timer for keeping the timestamp for streaming/ADC, DIO, etc.
