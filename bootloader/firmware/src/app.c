...
#define FORCE_BOOTLOADER_FLAG_ADDR 0x8007FFFC // Address must match what is defined in the project preprocessor definition (0x8007FFFC is the last word of data mem)
#define FORCE_BOOOT_VALUE 0x04CEB001    // magic force bootloader value
...
int APP_StartApp(void)
{
    ...
    DRV_USBHS_Deinitialize(sysObj.drvUSBObject);
    //while(!DRV_USBHS_Status (sysObj.drvUSBObject)==0);
    ...
    SYS_DEVCON_Deinitialize(sysObj.sysDevcon);
...
void APP_Tasks (void)
{
    ...
    if (m_TimerCallback == SYS_TMR_HANDLE_INVALID)
    {
    ...
    if(bootloaderData.currentState==BOOTLOADER_WAIT_FOR_NVM)
    {
