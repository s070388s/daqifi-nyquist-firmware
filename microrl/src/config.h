...
#define _COMMAND_LINE_LEN (1+512)									// for 32 chars
...
#define _PROMPT_DEFAULT "DAQIFI> "

/*
Defines the printed prompt length. Due to escape sequences this may not be the length of the string
(though that is usually a good guess)
 */
#define _PROMPT_DEFAULT_LEN strlen(_PROMPT_DEFAULT)
...
/*
 * DM (10/14/15): Added the ability to disable the command line splitting
 * This allows us to use microrl with libscpi, which contains its own parsing.
 */
// #define USE_SPLIT
