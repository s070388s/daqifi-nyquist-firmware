...
// Interrupt on change for AD7609
void IntHandlerChangeNotification_PortB(void)
{
    ++g_BoardData.InISR;
    
    PLIB_PORTS_ChannelChangeNoticeDisable(PORTS_ID_0, PORT_CHANNEL_B, 0x8);
    ...
    const AInModule* module = ADC_FindModule(&g_BoardConfig.AInModules, AIn_AD7609);
    ADC_ConversionComplete(module);
    
    --g_BoardData.InISR;
}

// Interrupt on change for AD7195
void IntHandlerChangeNotification_PortF(void)
{
    ++g_BoardData.InISR;
    
    PLIB_PORTS_ChannelChangeNoticeDisable(PORTS_ID_0, PORT_CHANNEL_F, 0x4);
    PLIB_PORTS_Read(PORTS_ID_0, PORT_CHANNEL_F);
    PLIB_INT_SourceFlagClear(INT_ID_0,INT_SOURCE_CHANGE_NOTICE_F);
    
    // Tell the app to read the results
    const AInModule* module = ADC_FindModule(&g_BoardConfig.AInModules, AIn_AD7173);
    ADC_ConversionComplete(module);
...
void IntHandlerDrvAdcEOS(void)
{
    ...
    // Clear EOS interrupt flag in ADC reg
    int dummy=ADCCON2;
    UNUSED(dummy);
