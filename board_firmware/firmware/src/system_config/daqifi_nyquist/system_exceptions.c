...
void _general_exception_handler ( void )
{
    ...
    if (_excep_code == 0x05)
    {
        // This typically means we're out of memory. Calling any of the printf functions will send us into an inifinite exception loop
        SYS_DEBUG_MESSAGE(SYS_ERROR_FATAL, _cause_str);
    }
    else
    {
...
void __attribute__((nomips16)) _bootstrap_exception_handler(void)
{
    ...
    SYS_DEBUG_PRINT(SYS_ERROR_FATAL, "\nBootstrap Exception %s (cause=%d, addr=%x).\n", 
                    _cause_str, _excep_code, _excep_addr);
...
void _simple_tlb_refill_exception_handler(void)
{
    ...
    SYS_DEBUG_PRINT(SYS_ERROR_FATAL, "\nTLB Exception %s (cause=%d, addr=%x).\n", 
                    _cause_str, _excep_code, _excep_addr);
...
void _cache_err_exception_handler(void)
{
    ...
    SYS_DEBUG_PRINT(SYS_ERROR_FATAL, "\nCache Exception %s (cause=%d, addr=%x).\n", 
                    _cause_str, _excep_code, _excep_addr);
