/*! @file DAC7718.c
 * @brief Implementation of the DAC7718 driver functions
 * 
...
/**
 * Creates a new configuration for the specified set of
 * inputs and returns the id
 * @param channel
 * @param pin
 * @param isInput
 * @return 
 */
uint8_t DAC7718_NewConfig(const DAC7718Config *pNewDAC7718Config)
{
    uint8_t id = m_DAC7718ConfigCount;
    ++m_DAC7718ConfigCount;
    
    if( m_DAC7718ConfigCount <= MAX_DAC7718_CONFIG ){
        memcpy(&m_DAC7718Config[id], pNewDAC7718Config, sizeof(DAC7718Config));
    }
    else{
        return 0xFF;
    }
