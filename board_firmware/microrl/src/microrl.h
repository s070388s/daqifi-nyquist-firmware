...
struct s_microrl_t;
// typedef s_microrl_t microrl_t;
typedef int (*MicrorlExecuteFxn)(struct s_microrl_t* pThis, size_t dataLen, const char * data);
typedef char** (*MicrorlCompleteFxn)(struct s_microrl_t* pThis, int argc, const char * const * argv);
typedef void (*MicrorlPrintFxn)(struct s_microrl_t* pThis, size_t dataLen, const char * data);


// microrl struct, contain internal library data
typedef struct s_microrl_t {
    ...
    size_t prompt_size;                // The size of the (printed) prompt string
    int terminalPos;                   // The terminal position
        
	char cmdline [_COMMAND_LINE_LEN];  // cmdline buffer
	int cmdlen;                        // last position in command line
	int dataCursor;                    // The current index into cmdline.
    
	MicrorlExecuteFxn execute; // ptr to 'execute' callback
	MicrorlCompleteFxn get_completion; // ptr to 'completion' callback
	MicrorlPrintFxn print;                // ptr to 'print' callback
    
#ifdef _USE_CTLR_C
	void (*sigint) (void);
#endif
    
    // Tracks the last processed character (which, due to escape sequences etc, is not always the last character in cmdline)
    // This allows us to uniformly handle \r, \n, \r\n or \n\r without resulting in double newlines in the last two cases.
    // \n\n WILL create double newlines though
    char lastChar;
    
    /**
     * Indicates whether echo is enabled for this client
     */
    int echoOn;
} microrl_t;
